...
export const DashboardUserCard = ({ data, index }) => { 
  ...
  {isUserRoleUpdated && (
    ...
    <p className='text-textColor text-[12px] font-semibold'>Are you sure, do you want to mark this user as <span>{data.role === "admin" ? "Member" : "Admin"}</span> ? </p>
    <div className='flex items-center gap-4'>
      <motion.button whileTap={{scale: 0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-blue-200 text-black hover:shadow-md' onClick={() => updateUserRole(data._id, data.role === "admin" ? "member" : "admin")}> Yes
      </motion.button>
      <motion.button whileTap={{scale: 0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-gray-200 text-black hover:shadow-md' onClick={() => setisUserRoleUpdated(false)}> No
      </motion.button>
    </div>
  </motion.div>
  )}
